...
| Student conversation history or device label disappears after refresh | Restore local history and the saved computer name while sending only the latest 10 messages | `test_student_page_persists_history_and_collects_computer_identity` |
...
| A teacher cannot identify which classroom computer made a request | Store the student-supplied computer name and request IP while retaining teacher ownership isolation | `test_teacher_can_view_only_owned_identified_classroom_records`, `test_classroom_record_schema_adds_computer_identity_without_deleting_old_data` |
...
3. Complete administrator setup, model API probing, class start, student-link access, streaming responses, generation cancellation, token rotation, and class end; confirm ending the class does not stop the teacher console.
...
9. Open Records as both a regular teacher and an administrator; verify ownership isolation, computer-name/IP and activity filters, rotation finalization, and deletion.
