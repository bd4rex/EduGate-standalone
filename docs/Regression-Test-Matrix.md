...
| Every upstream model must be entered manually, a large catalog stretches the System page, or a saved key cannot be reused | Keep only source settings and imported models on the main page; fetch `/models` into a searchable modal with editable display names and batch selection, reusing a saved key for the same provider configuration | `test_provider_models_can_be_discovered_and_batch_imported`, `test_upstream_models_can_be_discovered_selected_and_batch_imported` |
| A model name is treated as globally unique, so identical model IDs from different providers overwrite each other or reuse the wrong key | Separate the local catalog ID, provider ID, and upstream model ID; classroom policy references the local ID and routing restores the correct provider-native model ID | `test_same_upstream_model_from_two_providers_isolated_and_routes_native_id`, `test_runtime_config_backfills_legacy_model_provider_identity` |
| A provider can be added but not removed as a unit, or removal leaves models, keys, and invalid classroom references behind | Provider-level deletion removes every model and key together; referenced classroom policies must switch to another provider model, otherwise the API returns HTTP 409 | `test_provider_delete_removes_its_models_and_switches_active_reference`, `test_upstream_models_can_be_discovered_selected_and_batch_imported` |
...
3. Complete administrator setup; configure two providers that expose an identical model ID, verify provider switching, composite labels, endpoint composition, modal search, display-name edits, and batch import, then select, route, and delete one provider's model without affecting the other. Continue through class start, student-link access, streaming responses, generation cancellation, token rotation, and class end; confirm ending the class does not stop the teacher console.
